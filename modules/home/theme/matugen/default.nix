...

{
  ...
  config = lib.mkIf (config.host.theme.enable && config.host.theme.matugen.enable) (
    let
      fallbackWallpaper = builtins.path {
        path = "${toString ../../../../homes/x86_64-linux}/maddie@saturn/wallpapers/catppuccin.png";
        name = "matugen-fallback.png";
      };
      wallpaper =
        if config.host.theme.wallpaper != null then
